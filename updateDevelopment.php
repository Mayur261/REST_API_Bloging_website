    ...
    if (isset($_POST['id']) && isset($_POST['title']) && isset($_POST['editor']) && isset($_POST['topic'])) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($image_tmp_name, $image_destination)) {
                ...
                $sql = "UPDATE development SET title = ?, body = ?, topic = ?, image = ? WHERE id = ?";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssssi", $title, $body, $topic, $image_name, $id);

                if ($stmt->execute()) {
                    // Post updated successfully
                    header("Location: Development.php"); // Redirect to a post list page or another appropriate location
                    exit();
                } else {
            ...
            $sql = "UPDATE development SET title = ?, body = ?, topic = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
